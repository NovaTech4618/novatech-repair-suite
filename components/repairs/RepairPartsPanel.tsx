...
export default function RepairPartsPanel({ repairId }: { repairId: string }) {
  ...
  return <Card><CardHeader><CardTitle className="text-base sm:text-lg">Parts & engineer accountability</CardTitle></CardHeader><CardContent className="space-y-4">
    ...
    <div className="grid gap-3 rounded-2xl border bg-muted/20 p-3 sm:grid-cols-[1fr_100px_1fr_auto] sm:items-end"><label className="text-sm font-medium">Part<select value={inventoryId} onChange={(e) => setInventoryId(e.target.value)} className="mt-1 h-10 w-full rounded-xl border bg-background px-3 text-sm"><option value="">Select inventory item</option>{inventory.map((item) => <option key={item.id} value={item.id} disabled={item.quantity <= 0}>{item.item_name} — {item.quantity} in stock</option>)}</select></label><label className="text-sm font-medium">Qty<Input className="mt-1" type="number" min="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></label><label className="text-sm font-medium">Note<Input className="mt-1" placeholder="e.g. Screen replacement" value={notes} onChange={(e) => setNotes(e.target.value)} /></label><Button onClick={handleUse} disabled={saving || !inventoryId}>{saving ? "Issuing..." : "Issue Part"}</Button></div>
