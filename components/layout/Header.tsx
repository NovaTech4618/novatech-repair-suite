...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-20 items-center justify-between gap-2 border-b border-slate-200 bg-white px-3 sm:gap-4 sm:px-4 md:px-8">
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
        <SidebarTrigger className="shrink-0 rounded-xl text-slate-600 hover:bg-slate-100 md:hidden" />
      ...
      <div className="flex shrink-0 items-center gap-2 md:gap-3">
        <button type="button" onClick={() => router.push("/search")} className={`${iconButtonClass} hidden sm:inline-flex`} aria-label="Search"><Search className="h-4 w-4" /></button>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-2 py-2 shadow-sm sm:px-3">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-900 font-heading font-semibold text-white">{initial}</div>
        ...
        <button type="button" onClick={() => router.push("/settings")} className={`${iconButtonClass} hidden sm:inline-flex`} aria-label="Settings"><Settings className="h-4 w-4" /></button>
        <button type="button" onClick={handleLogout} className={`${iconButtonClass} hidden sm:inline-flex`} aria-label="Logout"><LogOut className="h-4 w-4" /></button>
      </div>
    </header>
  );
}
