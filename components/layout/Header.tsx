...
export default function Header() {
  ...
  useEffect(() => { fetchProfile(); loadAlerts(); }, [pathname]);
  ...
  const iconButtonClass = "inline-flex size-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 transition-colors hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#12b76a]";
    ...
    <header className="sticky top-0 z-40 flex min-h-16 items-center justify-between gap-2 border-b border-slate-200 bg-white/95 px-3 backdrop-blur sm:px-4 md:min-h-[72px] md:px-8">
      <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="hidden max-w-[45vw] truncate text-xs text-slate-500 sm:block">{companyName || "Your workspace"}</p>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
        ...
        <div className="flex size-9 items-center justify-center rounded-lg bg-slate-900 font-heading font-semibold text-white sm:size-auto sm:gap-2 sm:px-2 sm:py-2">
          <div className="flex size-7 items-center justify-center rounded-md bg-[#12b76a] font-heading text-sm font-bold text-white">{initial}</div>
          <div className="hidden min-w-0 text-left sm:block"><p className="max-w-36 truncate text-sm font-semibold text-slate-900">{displayName}</p><p className="max-w-36 truncate text-xs text-slate-500">{companyName || "Account"}</p></div>
        </div>
