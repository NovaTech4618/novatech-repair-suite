...
const CATEGORIES = [
  { value: "Part", label: "Part", hint: "Repair components issued to engineers" },
  { value: "Accessory", label: "Accessory", hint: "Retail accessories — not engineer-issued" },
  { value: "Gadget", label: "Gadget", hint: "Devices and electronics — retail sales" },
];

export default function InventoryForm({ editingItem, onSaved, onCancelEdit }: InventoryFormProps) {
  const [itemName, setItemName] = useState(""); const [category, setCategory] = useState("Part"); const [brand, setBrand] = useState("");
  ...
  function resetForm() { setItemName(""); setCategory("Part"); setBrand(""); setCompatibleModels(""); setSku(""); setSellingPrice(""); setCostPrice(""); setQuantity("0"); setMinimumStock("5"); setSupplier(""); setShelfLocation(""); setNotes(""); }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!CATEGORIES.some((item) => item.value === category)) return toast.error("Select a valid inventory category.");
    ...
    setLoading(true);
    const payload = { item_name: itemName.trim(), category, brand: brand.trim() || null, compatible_models: compatibleModels.trim() || null, sku: sku.trim() || null, selling_price: parsedSellingPrice, cost_price: parsedCostPrice, quantity: parsedQuantity, minimum_stock: parsedMinimumStock, supplier: supplier.trim() || null, shelf_location: shelfLocation.trim() || null, notes: notes.trim() || null };
  ...
  return <Card><CardHeader><CardTitle>{editingItem ? "Edit Inventory Item" : "Add Inventory Item"}</CardTitle></CardHeader><CardContent><form onSubmit={handleSubmit} className="space-y-4">
    <Input placeholder="Item Name" value={itemName} onChange={(e) => setItemName(e.target.value)} />
    <div><label className="mb-1 block text-sm font-medium text-slate-700">Inventory Type</label><select value={category} onChange={(e) => setCategory(e.target.value)} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-teal-500">{CATEGORIES.map((item) => <option key={item.value} value={item.value}>{item.label} — {item.hint}</option>)}</select></div>
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Input placeholder="Brand" value={brand} onChange={(e) => setBrand(e.target.value)} /><Input placeholder="SKU" value={sku} onChange={(e) => setSku(e.target.value)} /></div>
