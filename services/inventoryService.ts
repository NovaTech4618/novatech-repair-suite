...
export const inventoryService = {
  ...
  async addInventoryItem(item: InventoryItemInput) { return await supabase.from("inventory").insert([item]).select("*").single(); },
  ...
  async uploadItemImage(companyId:string,itemId:string,file:File) {
    ...
    const ext=(file.name.split(".").pop()||"jpg").toLowerCase().replace(/[^a-z0-9]/g,""); const path=`${companyId}/${itemId}.${ext}`;
    const {error}=await supabase.storage.from("inventory-images").upload(path,file,{upsert:true,contentType:file.type,cacheControl:"3600"}); if(error)return{data:null,error};
    const {data}=supabase.storage.from("inventory-images").getPublicUrl(path); const {error:updateError}=await supabase.from("inventory").update({image_url:data.publicUrl,updated_at:new Date().toISOString()}).eq("id",itemId); if(updateError)return{data:null,error:updateError}; return{data:data.publicUrl,error:null};
  },
