...
begin
  ...
  if v_company_id is null then raise exception 'Your account is not connected to a company'; end if;
  ...
  if nullif(trim(coalesce(p_phone, '')), '') is null then raise exception 'Customer phone is required'; end if;
  ...
  v_payment_method := case lower(trim(coalesce(p_payment_method, 'cash')))
    when 'cash' then 'Cash' when 'transfer' then 'Transfer' when 'pos' then 'POS' when 'other' then 'Other' else null end;
  ...
  if v_customer_id is null then
    ...
    update public.customers set full_name = trim(p_customer_name)
  ...
  values (v_company_id, v_customer_id, coalesce(nullif(trim(p_device_type), ''), 'Phone'), trim(p_brand), trim(p_model), nullif(trim(coalesce(p_serial_number, '')), ''), nullif(trim(coalesce(p_color, '')), ''), null, null, trim(p_issue))
  ...
  values (v_company_id, v_branch_id, v_device_id, nullif(trim(coalesce(p_technician, '')), ''), trim(p_issue), null, null, null, coalesce(nullif(trim(p_priority), ''), 'Normal'), 0, v_payment_method, p_expected_completion_date, v_estimated, null, 'Received')
  returning id into v_repair_id;

  if v_deposit > 0 then
    insert into public.repair_payments (company_id, repair_id, amount, payment_method, payment_date, notes, recorded_by)
    values (v_company_id, v_repair_id, v_deposit, v_payment_method, now(), 'Walk-in repair deposit', auth.uid());
  end if;
