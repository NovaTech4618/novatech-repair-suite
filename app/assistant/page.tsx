...
export default function AssistantPage() {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      try {
        ...
        const access = await checkPremiumAccess(accessToken);
        if (!active) return;
        if (!access.ok) {
          setPremium(false);
          return;
        ...
        if (access.premium && savedConversationId) {
          const history = await getPremiumConversation(accessToken, savedConversationId);
          if (!active) return;
  ...
  async function send(value = input) {
    ...
    if (!text || busy || !token || premium !== true) return;
    setInput("");
