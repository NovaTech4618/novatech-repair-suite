...
export default function FinancePage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const result = await financeService.createTransaction({
      direction, category, amount: value, payment_method: paymentMethod,
      description, occurred_at: new Date(occurredAt).toISOString(),
    });
    if (result.error) setError(result.error.message);
  ...
  return <main className="space-y-6 p-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h1 className="text-2xl font-bold tracking-tight">Finance</h1><p className="text-muted-foreground">Control money coming into and leaving the shop.</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-3">
      <div className="rounded-xl border p-5"><p className="text-sm text-muted-foreground">Money in</p><p className="mt-1 text-2xl font-bold">{money(totals.in)}</p></div>
      <div className="rounded-xl border p-5"><p className="text-sm text-muted-foreground">Money out</p><p className="mt-1 text-2xl font-bold">{money(totals.out)}</p></div>
      <div className="rounded-xl border p-5"><p className="text-sm text-muted-foreground">Net movement</p><p className="mt-1 text-2xl font-bold">{money(totals.in - totals.out)}</p></div>
    </div>

    <section className="rounded-xl border p-5">
      <h2 className="font-semibold">Record money movement</h2>
