...
const transactionLabel: Record<string, string> = {
  parts_out: "Parts collected", parts_in: "Parts returned", payment_in: "Payment received",
  payment_out: "Payment to engineer", opening_balance: "Opening balance",
  adjustment_debit: "Debit adjustment", adjustment_credit: "Credit adjustment",
};
...
export default function EngineersPage() {
  ...
  const [action, setAction] = useState<"parts" | "payment" | "opening" | null>(null);
  ...
  async function load() {
    ...
    const [engineersRes, balancesRes, inventoryRes] = await Promise.all([
      engineerService.getEngineers(), engineerService.getBalances(), inventoryService.getInventory(),
    ]);
    if (engineersRes.error) setError(engineersRes.error.message); else setEngineers((engineersRes.data ?? []) as Engineer[]);
    if (balancesRes.error) setError(balancesRes.error.message); else setBalances((balancesRes.data ?? []) as EngineerBalance[]);
    if (inventoryRes.error) setError(inventoryRes.error.message); else setInventory((inventoryRes.data ?? []) as InventoryItem[]);
    setLoading(false);
  }

  async function selectEngineer(id: string) {
    setSelectedId(id); setAction(null); setMessage(""); setError(""); setDetailLoading(true);
    const result = await engineerService.getTransactions(id);
    if (result.error) setError(result.error.message); else setTransactions((result.data ?? []) as EngineerTransaction[]);
  ...
  function resetAction() {
    setAction(null); setInventoryId(""); setQuantity(1); setUnitPrice(""); setAmount(""); setPaymentMethod("cash"); setNotes("");
  }

  function openCreate() { setEditingEngineer(null); setEngineerForm(emptyEngineer); setShowEngineerForm(true); setError(""); setMessage(""); }
  function openEdit(engineer: Engineer) {
    setEditingEngineer(engineer);
    setEngineerForm({ name: engineer.name, phone: engineer.phone ?? "", business_name: engineer.business_name ?? "", address: engineer.address ?? "", notes: engineer.notes ?? "" });
    setShowEngineerForm(true); setError(""); setMessage("");
  }

  async function submitEngineer(event: React.FormEvent) {
    event.preventDefault();
    if (!engineerForm.name?.trim()) { setError("Engineer name is required."); return; }
    ...
    setShowEngineerForm(false); setEngineerForm(emptyEngineer);
  ...
  async function toggleStatus(engineer: Engineer) {
    ...
    if (result.error) setError(result.error.message); else { setMessage(`${engineer.name} is now ${nextStatus}.`); await load(); }
    setSaving(false);
  }
  ...
  async function submitAction(event: React.FormEvent) {
    ...
    let result;
    if (action === "parts") {
      if (!inventoryId || quantity < 1) { setError("Select a part and enter a valid quantity."); setSaving(false); return; }
      result = await engineerService.recordPartsOut(selectedId, inventoryId, quantity, unitPrice ? Number(unitPrice) : undefined, notes.trim() || null);
    } else if (action === "payment") {
      const value = Number(amount);
      if (!value || value <= 0) { setError("Enter a valid payment amount."); setSaving(false); return; }
      result = await engineerService.recordPaymentIn(selectedId, value, paymentMethod, notes.trim() || null);
    } else {
      const value = Number(amount);
      if (!value || value <= 0) { setError("Enter a valid opening balance."); setSaving(false); return; }
      result = await engineerService.recordOpeningBalance(selectedId, value, notes.trim() || null);
    }
    if (result.error) { setError(result.error.message); setSaving(false); return; }
    setMessage(action === "parts" ? "Parts collection recorded successfully." : action === "payment" ? "Payment recorded successfully." : "Opening balance recorded successfully.");
    resetAction(); await Promise.all([load(), selectEngineer(selectedId)]); setSaving(false);
  }
  ...
  return (
    <main className="space-y-6 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h1 className="text-2xl font-bold tracking-tight">Engineers</h1><p className="text-muted-foreground">Track contract technicians, parts collected, returns and payments.</p></div>
      ...
      {showEngineerForm && (
        <form onSubmit={submitEngineer} className="rounded-xl border bg-muted/20 p-5">
          <div className="mb-4 flex items-center justify-between"><div><h2 className="font-semibold">{editingEngineer ? "Edit engineer" : "Add engineer"}</h2><p className="text-sm text-muted-foreground">Account history is preserved when an engineer is deactivated.</p></div><button type="button" onClick={() => setShowEngineerForm(false)} className="text-sm text-muted-foreground">Cancel</button></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)]">
        <section className="rounded-xl border">
          ...
          <div className="divide-y">
            {engineers.length === 0 ? <div className="p-8 text-center text-muted-foreground">No engineers found.</div> : engineers.map((engineer) => {
              ...
              return <div key={engineer.id} className={`p-4 transition hover:bg-muted/50 ${selectedId === engineer.id ? "bg-muted" : ""}`}>
        ...
        <section className="rounded-xl border">
          {!selectedEngineer ? <div className="flex min-h-[420px] items-center justify-center p-8 text-center text-muted-foreground">Select an engineer to view their ledger.</div> : <>
            ...
            <div className="p-5"><div className="mb-4 flex items-center justify-between"><h3 className="font-semibold">Transaction history</h3><span className="text-sm text-muted-foreground">{transactions.length} transactions</span></div>{detailLoading ? <p className="py-8 text-center text-muted-foreground">Loading ledger...</p> : transactions.length === 0 ? <p className="rounded-md border border-dashed p-8 text-center text-muted-foreground">No transactions yet.</p> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="p-3">Date</th><th className="p-3">Description</th><th className="p-3">Type</th><th className="p-3 text-right">Debit</th><th className="p-3 text-right">Credit</th><th className="p-3 text-right">Balance</th></tr></thead><tbody>{[...transactions].reverse().map((transaction, index, chronological) => { const running = chronological.slice(0, index + 1).reduce((sum, row) => sum + Number(row.debit) - Number(row.credit), 0); return <tr key={transaction.id} className="border-b last:border-0"><td className="whitespace-nowrap p-3">{new Date(transaction.transaction_date).toLocaleDateString()}</td><td className="p-3">{transaction.description}</td><td className="p-3">{transactionLabel[transaction.transaction_type] ?? transaction.transaction_type}</td><td className="p-3 text-right">{Number(transaction.debit) ? money(Number(transaction.debit)) : "—"}</td><td className="p-3 text-right">{Number(transaction.credit) ? money(Number(transaction.credit)) : "—"}</td><td className="p-3 text-right font-medium">{money(running)}</td></tr>; })}</tbody></table></div>}</div>
          </>}
        </section>
      </div>
    </main>
