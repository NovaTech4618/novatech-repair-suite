      ...
      <div className="mx-auto w-full max-w-7xl space-y-5 px-1 sm:space-y-6 sm:px-0">
          ...
          <CardContent className="p-3 sm:p-6">
            <div className="mb-4 flex flex-col gap-3 sm:mb-6 sm:flex-row">
              ...
              <select
                ...
                className="h-10 w-full rounded-2xl border border-transparent bg-input/50 px-3 text-sm outline-none sm:w-auto"
              >
                {statusOptions.map((s) => (
            ...
            {filtered.length === 0 ? (
              ...
              <>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filtered.map((repair) => {
                        ...
                        const total = Number(
                          repair.final_cost ?? repair.estimated_cost ?? 0
                        );
                        const paid = Number(repair.deposit ?? 0);
                ...
                {/* Mobile layout: cards instead of a wide table */}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Badge
                            className="shrink-0"
                            variant={statusVariant(repair.status)}
                          >
                            {repair.status}
                          </Badge>
                        </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                          ...
                          <div>
                            ...
                            {ticket ? (
                              <Link
                                href={`/tickets/${ticket.id}`}
                                className="mt-0.5 block truncate font-medium text-blue-600 hover:underline"
                              >
                                {ticket.ticket_number}
                              </Link>
                            ) : (
                          ...
                          <div>
                            ...
                            <p className="mt-0.5 font-semibold">{money(total)}</p>
                          </div>
                          <div>
                            <p className="text-xs text-muted-foreground">Paid</p>
                            <p className="mt-0.5 font-semibold">{money(paid)}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-3 rounded-xl bg-muted/50 p-3">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <span
                              className={
                                balance > 0 ? "font-bold" : "font-medium text-muted-foreground"
                              }
                            >
                              {money(balance)}
                            </span>
        ...
        <Card>
          <CardHeader className="p-4 sm:p-6">
            <CardTitle className="text-base sm:text-lg">Repair money flow</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4 text-sm text-muted-foreground sm:px-6 sm:pb-6">
...
type PaymentFormProps = {
  ...
  onSubmit: (repair: RepairRow) => void;
};
...
function PaymentForm({
  ...
  return (
    <div className="mt-3 w-full rounded-2xl border bg-muted/30 p-3 sm:p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <span className="text-xs text-muted-foreground">Outstanding</span>
        <span className="text-sm font-bold">{money(balance)}</span>
      </div>
      ...
      <Button
        ...
        disabled={saving}
        onClick={() => onSubmit(repair)}
      >
        {saving ? "Recording payment..." : `Confirm ${money(Number(amount) || 0)} Payment`}
      </Button>
    </div>
  );
}
