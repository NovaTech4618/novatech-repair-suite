...
function statusVariant(status: string) { if (status === "Completed" || status === "Collected") return "default"; if (status === "Repairing" || status === "Testing") return "secondary"; return "outline"; }
...
export default function RepairsPage() {
  ...
  async function fetchRepairs() {
    setLoading(true); await getCurrentSession();
    const { data, error } = await repairService.getAllRepairs(); setLoading(false);
  ...
  const filtered = useMemo(() => repairs.filter((repair) => {
    const query = search.trim().toLowerCase(); const device = repair.devices; const customer = device?.customers;
  ...
  const activeCount = repairs.filter((repair) => !["Completed", "Collected"].includes(repair.status)).length;
  const statusOptions = ["Received", "Diagnosis", "Estimate Sent", "Customer Approved", "Repairing", "Testing", "Completed", "Collected"];
        ...
        <Card className="overflow-hidden border-slate-200 shadow-sm">
          <CardContent className="p-0">
            <div className="flex flex-col gap-3 border-b border-slate-100 bg-slate-50/50 p-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-slate-700 outline-none focus:border-teal-400 focus:ring-2 focus:ring-teal-100 sm:w-52"><option value="all">All statuses</option>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select>
            </div>
            ...
            {loading ? <div className="space-y-3 p-5"><div className="h-12 animate-pulse rounded-xl bg-slate-100" /><div className="h-12 animate-pulse rounded-xl bg-slate-100" /><div className="h-12 animate-pulse rounded-xl bg-slate-100" /></div> : filtered.length === 0 ? <div className="p-12 text-center"><Wrench className="mx-auto size-8 text-slate-300" /><p className="mt-3 text-sm font-medium text-slate-700">{search || statusFilter !== "all" ? "No repairs match your search" : "No repairs yet"}</p><p className="mt-1 text-xs text-slate-500">Start a repair from the device desk.</p></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow className="bg-white"><TableHead>Device</TableHead><TableHead>Customer</TableHead><TableHead>Problem</TableHead><TableHead>Status</TableHead><TableHead>Balance</TableHead><TableHead className="w-10" /></TableRow></TableHeader><TableBody>{filtered.map((repair) => { const total = Number(repair.final_cost ?? repair.estimated_cost ?? 0); const paid = Number(repair.deposit ?? 0); const balance = Math.max(total - paid, 0); const ticket = repair.repair_tickets?.[0]; return <TableRow key={repair.id} className="group"><TableCell><Link href={`/repairs/${repair.id}`} className="block min-w-[190px]"><span className="font-semibold text-slate-900 group-hover:text-teal-700">{repair.devices?.brand} {repair.devices?.model}</span><span className="mt-0.5 block text-xs text-slate-400">{ticket?.ticket_number || "Repair job"}</span></Link></TableCell><TableCell className="text-sm text-slate-600">{repair.devices?.customers?.full_name || "Walk-in"}</TableCell><TableCell className="max-w-[360px] text-sm text-slate-600"><span className="line-clamp-2">{repair.issue || "No issue recorded"}</span></TableCell><TableCell><Badge variant={statusVariant(repair.status)}>{repair.status}</Badge></TableCell><TableCell className={`text-sm font-semibold ${balance > 0 ? "text-slate-900" : "text-slate-400"}`}>{money(balance)}</TableCell><TableCell><Link href={`/repairs/${repair.id}`} aria-label="Open repair" className="inline-flex size-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700"><ArrowRight className="size-4" /></Link></TableCell></TableRow>; })}</TableBody></Table></div>}
          </CardContent>
        </Card>
      </main>
    </AppLayout>
