...
export default function RepairsPage() {
  ...
  async function fetchRepairs() {
    ...
    const [{ data, error }, { data: balanceRows }] = await Promise.all([
      ...
      repairService.getAllRepairBalances(),
    ]);
    setLoading(false);
    if (error) { toast.error("Failed to load repairs."); return; }
    const balances = new Map(((balanceRows ?? []) as RepairBalance[]).map((row) => [row.repair_id, Number(row.outstanding ?? 0)]));
    setRepairs(((data as unknown as RepairRow[]) || []).map((repair) => ({ ...repair, outstanding: balances.get(repair.id) ?? 0 })));
  }
