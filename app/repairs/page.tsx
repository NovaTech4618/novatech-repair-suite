            ...
            {filtered.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">No repairs found.</p> : <>
                  ...
                  <TableBody>
                    {filtered.map((repair) => {
                      ...
                      return <TableRow key={repair.id}>
                        ...
                        <TableCell>{money(total)}</TableCell><TableCell>{money(paid)}</TableCell><TableCell className={balance > 0 ? "font-semibold" : "text-muted-foreground"}>{money(balance)}</TableCell>
                        <TableCell>{ticket ? <Link href={`/tickets/${ticket.id}`} className="text-blue-600 hover:underline">{ticket.ticket_number}</Link> : "-"}</TableCell>
                        <TableCell className="min-w-[300px]"><div className="flex flex-wrap gap-2"><Link href={`/repairs/${repair.id}/parts`}><Button size="sm" variant="outline" type="button">Parts Used</Button></Link>{balance > 0 && <Button size="sm" variant="secondary" onClick={() => openPayment(repair.id)}>{paymentOpen ? "Close" : "Record Payment"}</Button>}{!isDone && <Button size="sm" onClick={() => handleComplete(repair.id)} disabled={completingId === repair.id}>{completingId === repair.id ? "Completing..." : "Mark Completed"}</Button>}</div>{paymentOpen && balance > 0 && <PaymentForm repair={repair} balance={balance} amount={paymentAmount} method={paymentMethod} saving={paying} setAmount={setPaymentAmount} setMethod={setPaymentMethod} onSubmit={handlePayment} />}</TableCell>
                      </TableRow>;
                    })}
                  </TableBody>
                </Table>
              ...
              <div className="grid gap-3 md:hidden">
                {filtered.map((repair) => {
                  ...
                  const total = Number(repair.final_cost ?? repair.estimated_cost ?? 0); const paid = Number(repair.deposit ?? 0); const balance = Math.max(total - paid, 0);
                  const isDone = repair.status === "Completed" || repair.status === "Collected"; const paymentOpen = paymentRepairId === repair.id;
                  return <div key={repair.id} className="rounded-2xl border bg-background p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link href={`/devices/${repair.device_id}`} className="block truncate font-semibold text-blue-600 hover:underline">{device?.brand} {device?.model}</Link><p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{repair.issue}</p></div><Badge className="shrink-0" variant={statusVariant(repair.status)}>{repair.status}</Badge></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted-foreground">Customer</p><p className="mt-0.5 truncate font-medium">{customer?.full_name || "-"}</p></div><div><p className="text-xs text-muted-foreground">Ticket</p>{ticket ? <Link href={`/tickets/${ticket.id}`} className="mt-0.5 block truncate font-medium text-blue-600 hover:underline">{ticket.ticket_number}</Link> : <p className="mt-0.5">-</p>}</div><div><p className="text-xs text-muted-foreground">Total</p><p className="mt-0.5 font-semibold">{money(total)}</p></div><div><p className="text-xs text-muted-foreground">Paid</p><p className="mt-0.5 font-semibold">{money(paid)}</p></div></div><div className="mt-3 rounded-xl bg-muted/50 p-3"><div className="flex items-center justify-between gap-3"><span className="text-sm text-muted-foreground">Outstanding</span><span className={balance > 0 ? "font-bold" : "font-medium text-muted-foreground"}>{money(balance)}</span></div><p className="mt-1 text-xs text-muted-foreground">{balance === 0 && total > 0 ? "Fully paid" : paid > 0 ? "Partially paid" : "No payment recorded"}</p></div><div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2"><Link href={`/repairs/${repair.id}/parts`} className="w-full"><Button type="button" className="w-full" variant="outline">Parts Used</Button></Link>{balance > 0 && <Button type="button" className="w-full" variant="secondary" onClick={() => openPayment(repair.id)}>{paymentOpen ? "Close Payment" : "Record Payment"}</Button>}{!isDone && <Button type="button" className="w-full" onClick={() => handleComplete(repair.id)} disabled={completingId === repair.id}>{completingId === repair.id ? "Completing..." : "Mark Completed"}</Button>}</div>{paymentOpen && balance > 0 && <PaymentForm repair={repair} balance={balance} amount={paymentAmount} method={paymentMethod} saving={paying} setAmount={setPaymentAmount} setMethod={setPaymentMethod} onSubmit={handlePayment} />}</div>;
                })}
              </div>
            </>}
          </CardContent>
...
function PaymentForm({ repair, balance, amount, method, saving, setAmount, setMethod, onSubmit }: PaymentFormProps) {
  return <div className="mt-3 w-full rounded-2xl border bg-muted/30 p-3 sm:p-4"><div className="mb-3 flex items-center justify-between gap-3"><span className="text-xs text-muted-foreground">Outstanding</span><span className="text-sm font-bold">{money(balance)}</span></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><div><label className="mb-1.5 block text-xs font-medium text-muted-foreground">Payment amount</label><Input type="number" inputMode="decimal" min="0.01" max={balance} step="0.01" placeholder="Enter amount" value={amount} onChange={(e) => setAmount(e.target.value)} className="h-10 w-full" /></div><div><label className="mb-1.5 block text-xs font-medium text-muted-foreground">Payment method</label><select value={method} onChange={(e) => setMethod(e.target.value)} className="h-10 w-full rounded-xl border bg-background px-3 text-sm outline-none focus:ring-2 focus:ring-ring/30"><option>Cash</option><option>Transfer</option><option>POS</option><option>Other</option></select></div></div><Button type="button" className="mt-3 h-10 w-full" disabled={saving} onClick={() => onSubmit(repair)}>{saving ? "Recording payment..." : `Confirm ${money(Number(amount) || 0)} Payment`}</Button></div>;
}
