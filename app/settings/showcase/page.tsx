...
export default function ShowcaseSettingsPage() {
  ...
  return <AppLayout><div className="space-y-6"><Link href="/settings" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600 hover:text-slate-950"><ArrowLeft className="size-4" />Back to Settings</Link><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-teal-600">Business identity</p><h1 className="mt-1 font-heading text-3xl font-bold tracking-tight text-slate-950">Your public presence</h1><p className="mt-1 text-sm text-slate-500">Control what customers see outside your private workspace.</p></div><CompanyShowcaseSettings companyId={data.company_id} initialSlug={company.slug || ""} initialEnabled={company.showcase_enabled || false} initialDescription={company.showcase_description} initialPhone={company.showcase_phone} initialAddress={company.showcase_address} initialServices={company.showcase_services || []} /></div></AppLayout>;
}
