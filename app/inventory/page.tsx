...
export default function InventoryPage() {
  ...
  useEffect(() => {
    inventoryService.getInventory().then((result) => {
      if (!result.error) setItems((result.data ?? []) as InventoryItem[]);
    });
  }, [refreshKey]);

  const filtered = useMemo(() => items.filter((item) => {
    const text = `${item.item_name} ${item.brand ?? ""} ${item.sku ?? ""} ${item.shelf_location ?? ""}`.toLowerCase();
    const matchesQuery = text.includes(query.toLowerCase());
    const matchesStock = stockFilter === "all" || (stockFilter === "low" ? item.quantity <= item.minimum_stock : item.quantity === 0);
    return matchesQuery && matchesStock;
  }), [items, query, stockFilter]);

  const lowStock = items.filter((item) => item.quantity <= item.minimum_stock);
  ...
  return <AppLayout>
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search part, SKU, brand or shelf..." className="h-10 flex-1 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-teal-500" /><select value={stockFilter} onChange={(e) => setStockFilter(e.target.value)} className="h-10 rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-teal-500"><option value="all">All stock</option><option value="low">Low stock</option><option value="empty">Out of stock</option></select></div>
        <div className="mt-4 flex flex-wrap gap-6 text-sm text-slate-500"><span><strong className="text-slate-900">{items.length}</strong> parts</span><span><strong className="text-amber-700">{lowStock.length}</strong> need attention</span><span><strong className="text-slate-900">{filtered.length}</strong> shown</span></div>
      </section>
